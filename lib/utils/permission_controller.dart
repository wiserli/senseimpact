...
class PermissionsController {
  ...
  Future<bool> _checkPermissions() async {
    ...
    final statusMap = {Permission.camera: cameraStatus};
    if (Platform.isIOS) {
    ...
    if (statusMap.values.every((status) => status.isGranted)) {
      ...
      final statusMap2 = await requestPermissions();
      return statusMap2.values.every((status) => status.isGranted);
    }
  }

  Future<Map<Permission, PermissionStatus>> requestPermissions() async {
    final statusList = [Permission.camera, Permission.notification];
  ...
  // Optional: Request notification permission separately
